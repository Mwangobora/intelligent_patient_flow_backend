...
@pytest.fixture
def organization():
    return Organization.objects.create(name="Laboratory Org", code="LAB_ORG")


@pytest.fixture
def facility_type():
    return FacilityType.objects.create(name="Lab Hospital", code="LAB_HOSP")
...
def department(facility):
    return Department.objects.create(facility=facility, name="Outpatient", code="LAB_OPD")


@pytest.fixture
def specialty():
    return Specialty.objects.create(name="Laboratory General Medicine", code="LAB_GEN_MED")
...
def practitioner_assignment(organization, facility):
    practitioner_type = PractitionerType.objects.create(
        name="Lab Doctor", code="LAB_DOCTOR", requires_license=True
    )
    practitioner = Practitioner.objects.create(
        organization=organization,
        practitioner_type=practitioner_type,
        practitioner_number="LAB-DOC-001",
        first_name="Asha",
        last_name="Mollel",
    )
